...
async def test_modify(jp_fetch):
    ...
    diff = {"baz": 75, "wib": [1, 2, 3], "sub": {"a": 8, "d": 9}}

    await jp_fetch("api", "config", "example", method="PUT", body=json.dumps(sample))

    response2 = await jp_fetch(
        "api", "config", "example", method="PATCH", body=json.dumps(modified_sample)
    )
