...
from jupyter_server.config_manager import BaseJSONConfigManager, StrDict, recursive_update
...
class ConfigManager(LoggingConfigurable):
    ...
    def get(self, section_name: str) -> StrDict:
    ...
    def update(self, section_name: str, new_data: StrDict) -> StrDict:
    ...
    def _default_write_config_manager(self) -> BaseJSONConfigManager:
        return BaseJSONConfigManager(config_dir=self.write_config_dir)

    @observe("write_config_dir")
    def _update_write_config_dir(self, change: t.Any) -> None:
